...
router.put("/me", auth, async (req, res) => {
    try {
        ...
        const emailChanged =
            nextEmail !== user.email && nextEmail !== user.pendingEmail;
    ...
    try {
        ...
        if (!user.pendingEmail) {
            return res.status(400).json({
                msg: "There is no email address waiting for verification",
            });
        }
...
router.get("/verify-email/:token", async (req, res) => {
    try {
        ...
        if (!user || !user.pendingEmail) {
            return res.redirect(
                `${getFrontendBaseUrl()}/settings?verification=invalid`,
            );
        }
        ...
        if (emailAlreadyTaken) {
            return res.redirect(
                `${getFrontendBaseUrl()}/settings?verification=taken`,
            );
        }
        ...
        return res.redirect(
            `${getFrontendBaseUrl()}/settings?verification=success`,
        );
    } catch (err) {
        console.error(err);
        return res.redirect(
            `${getFrontendBaseUrl()}/settings?verification=error`,
        );
    }
});
        ...
        try {
            ...
            if (String(req.user._id) !== String(userId)) {
                return res.status(403).json({
                    msg: "You can only update your own profile image",
                });
            }
    ...
    async (req, res) => {
        try {
            ...
            if (String(req.user._id) !== String(userId)) {
                return res
                    .status(403)
                    .json({ msg: "You can only update your own header image" });
            }
...
router.post("/bio", auth, async (req, res) => {
    try {
        ...
        if (String(req.user._id) !== String(userId)) {
            return res
                .status(403)
                .json({ msg: "You can only update your own bio" });
        }
...
router.put("/me", auth, async (req, res) => {
    try {
        const userId = req.user._id;
        const { username, email, bio, notificationsEnabled } = req.body;
        ...
        if (duplicateUser) {
            if (
                duplicateUser.username === nextUsername &&
                duplicateUser.email === nextEmail
            ) {
                return res
                    .status(400)
                    .json({ msg: "Username and email already exist" });
            }
