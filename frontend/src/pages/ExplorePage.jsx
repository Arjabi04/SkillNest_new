...
function ExplorePage() {
    ...
    const peopleYouMayKnow = Object.values(
        allPosts.reduce((acc, post) => {
            ...
            if (
                !user ||
                !userKey ||
                userKey.toString() === actingUserId.toString()
            )
                return acc;
            if (!acc[userKey]) {
                acc[userKey] = {
    ...
    const applyFeedFilters = (
        sourcePosts,
        tagValue = selectedTrendingTag,
        queryValue = searchQuery,
    ) => {
        const normalizedTag = String(tagValue || "")
            .trim()
            .toLowerCase();
        const normalizedQuery = String(queryValue || "")
            .trim()
            .toLowerCase();
    ...
    const handleDeleteComment = async (postId, commentIdx) => {
        ...
        try {
            const res = await fetch(
                `${API_URL}/posts/${postId}/comments/${commentIdx}`,
                {
                    method: "DELETE",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ userId: actingUserId }),
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans flex">
            ...
            {/* Main content */}
            <div
                className={`flex-1 ${mainContentClass} max-w-300 mx-auto px-6 py-8`}>
                <PageHeader
                    className="mb-8"
                        ...
                        {loading ? (
                            ...
                            <div className="rounded-2xl bg-white border border-slate-200 p-12 text-center">
                                ...
                                <p className="text-slate-500">
                                    Be the first to share something on
                                    SkillNest!
                                </p>
                            </div>
                        ) : (
                    ...
                    <aside className="hidden lg:block w-80 space-y-5 self-start sticky top-6 max-h-[calc(100vh-2rem)] overflow-y-auto pr-1">
                        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                            ...
                            {trendingTopics.length === 0 ? (
                                <p className="text-xs text-slate-500">
                                    No topics yet. Add tags to posts to see
                                    trends.
                                </p>
                            ) : (
                                <div className="space-y-1">
                                    {trendingTopics.map(([tag, count]) => (
                                        <button
                                            ...
                                            key={tag}
                                            onClick={() =>
                                                handleTrendingTopicClick(tag)
                                            }
                                            className={`w-full flex items-center justify-between px-3 py-2 rounded-lg transition-colors ${
                                                selectedTrendingTag.toLowerCase() ===
                                                tag.toLowerCase()
                                                    ? "bg-blue-50 border border-blue-200"
                                                    : "hover:bg-slate-50"
                                            }`}>
                                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full text-slate-700 bg-slate-100">
                                                #{tag}
                                            </span>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                            ...
                            {peopleYouMayKnow.length === 0 ? (
                                <p className="text-xs text-slate-500">
                                    Not enough activity yet to suggest people.
                                </p>
                            ) : (
